...
//SprEffectInfo 초기화
void EffectManager::initEffect(EffectName spr_effect, const char* plist_tex_name, int frame_num)
{
	if (EffectTable.find(spr_effect) != EffectTable.end())
		removeEffect(spr_effect);

	//set의 key로 저장하기 위해, string으로 convert
	string casted_name = string(plist_tex_name);
	Texture2D* tex_in_cache = TextureCache::getInstance()->getTextureForKey("effect/" + casted_name + ".png");

	if (tex_in_cache == NULL) {
		tex_in_cache = TextureCache::getInstance()->addImage("effect/" + casted_name + ".png");
		SpriteFrameCache::getInstance()->addSpriteFramesWithFile("effect/" + casted_name + ".plist", tex_in_cache);
	}

	auto effect_info = new SprEffectInfo(casted_name, *tex_in_cache);
	for (int i = 0; i < frame_num; i++) {
		effect_info->frames.pushBack(SpriteFrameCache::getInstance()->getSpriteFrameByName(StringUtils::format("Effect_%d_%i.png", (int)spr_effect, i + 1)));
	}
	EffectTable.insert({ spr_effect , effect_info });
}



//labelEffectInfo 초기화
void EffectManager::initEffect(EffectName label_effect, const char* tex_name)
{
	if (EffectTable.find(label_effect) != EffectTable.end())
		removeEffect(label_effect);

	//set의 key로 저장하기 위해, string으로 convert
	string casted_name = string(tex_name);
	Texture2D* tex_in_cache = TextureCache::getInstance()->getTextureForKey("effect/" + casted_name + ".png");
	...
	auto effect_info = new LabelEffectInfo(casted_name, *tex_in_cache);
	EffectTable.insert({ label_effect , effect_info });
}


void EffectManager::removeEffect(EffectName effect_name)
{
	if (EffectTable.find(effect_name) != EffectTable.end())
	{
		auto removed_address = EffectTable[effect_name]->address;
		delete(EffectTable[effect_name]);
		EffectTable.erase(effect_name);

		//더이상 참조하지 않는 리소스일 경우, 캐쉬에서도 해제
...
void EffectManager::displayAtkScore(int score, Point position, float size, Layer& target_canvas)
{
	if (EffectTable[SCORE_NORMAL])
	{
		...
		if (casted_effect.labelEffect == NULL) {
			...
			casted_effect.labelEffect->setScale(1.3f);
